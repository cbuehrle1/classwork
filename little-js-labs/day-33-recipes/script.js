...
function returnWithThumbnail(data){
...
var result = returnWithThumbnail(apiResult);
...
function containsButter(data){
  var output = []

  for (var i = 0; i < data.results.length; i++){
    var recipe = data.results[i];

    if (recipe.ingredients.indexOf("butter") != -1){
      output.push(recipe);
    }
  }
  return output;
...
var result = containsButter(apiResult);

// console.log(result)

//Write a function that will return all recipes that include mushrooms

function containsMushrooms(data){
  var output = []

  for (var i =0; i < data.results.length; i++){
    var recipe = data.results[i];

    if (recipe.ingredients.indexOf("mushroom") != -1){
      output.push(recipe);
    }
  }
  return output;
...
var result = containsMushrooms(apiResult);

// console.log(result)

//Once those two are complete, refactor so that they can both use the same function

function recipeIngredientSearch(data, item){
  var output = []

  for (var i = 0; i < data.results.length; i++){
    var recipe = data.results[i];

    if (recipe.ingredients.indexOf(item) != -1){
      output.push(recipe);
    }
  }
  return output;
...
var result = recipeIngredientSearch(apiResult, "butter")
...
function recipesLongerThanTen(data){
  var output = []

  for (var i = 0; i < data.results.length; i++){
    var recipe = data.results[i];

    var ingArr = recipe.ingredients.split(", ");
...
var result = recipesLongerThanTen(apiResult);

// console.log(result)

//Write a function that will return all recipes with more than 13 ingredients

function recipesLongerThanThirteen(data){
  var output = []
...
var result = recipesLongerThanThirteen(apiResult)

// console.log(result)

//Once those two are complete, refactor so that they can both use the same function

function filterByIngredientAmt(data, num){
  var output = []
...
var result = filterByIngredientAmt(apiResult, 13)
